...
algU' a@(Type x) b@(Type _) | a == b = []
                            | otherwise = [TS x b]
algU' a@(Type x) b   | x `elem` freeTypes b = error "error"
                    | otherwise            = [TS x b] 
algU' a b@(Type _) = algU' b a
algU' (Arrow a b) (Arrow c d) = algU' (subsT (algU' b d) a) (subsT (algU' b d) c) `composeSubs` algU' b d

algU :: [(Type, Type)] -> Substitution
algU [] = []
algU ((Arrow a b, Arrow c d):xs) = algU ((a, c) : (b, d) : xs)
algU ((a, b):xs) = ab ++ algU (map (\(x, y) -> ((subsT ab x), (subsT ab y))) xs) 
                    where ab = algU' a b
...
algE' g (Var x) a _ = let x' = lookupContext x g
                        in case x' of
                            Just f -> (a, f):[]
                            Nothing -> []
algE' g (App a b) t arr =   let fr = fresh $ arr
                            in algE' g a (Arrow (Type fr) t) (fr:arr) ++ algE' g b (Type fr) (fr:arr)
algE' g@(Context ctx) (Lam a b) t arr = let fo = fresh arr
                                            ft = fresh $ fo:arr
                                        in (t, Arrow (Type fo) (Type ft)) : algE (Context ((a, Type fo) : ctx)) b (Type ft)

algPP :: Term -> (Context, Type)
algPP term = let func = algU . algE (Context gamma0) term $ sigma0
                 fr = free term
